...
def get_task_arn(cluster_name, task_definition_name):
    ...
    task_definition = ecs_client.describe_task_definition(taskDefinition=task_definition_name)["taskDefinition"]
...
def terminate_task(cluster, task_arn, reason):
    logging.info("terminating task cluster:%s arn:%s reason:%s" % (cluster, task_arn, reason))
...
def terminate(success):
    ...
    if task_arn:
        terminate_task(cluster_name, task_arn, "Queue drained" if success else "Timeout")
    else:
...
if __name__ == "__main__":
    configure_logging()
    for key in ("rabbitmq_management_host", "rabbitmq_management_user", "rabbitmq_management_password", "ECS_CLUSTER", "TASK_DEFINITION_NAME"):
